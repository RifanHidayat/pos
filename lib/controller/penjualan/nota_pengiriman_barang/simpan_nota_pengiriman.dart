...
class SimpanNotaPengirimanController extends GetxController {
  ...
  Future<bool> simpanDODT(List dataSelected) {
    List filterDataUpdated = [];
    for (var element in notaPengirimanBarangCt.barangTerpilih) {
      if (Utility.validasiValueDouble("${element['qty_beli']}") > 0.0) {
        filterDataUpdated.add(element);
      }
    }
    print(filterDataUpdated);
